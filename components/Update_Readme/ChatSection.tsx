...
const ChatSection = ({
  ...
  const { setShowModel, selectedModel, showModel, storedUser, setStoredUser } =
      ...
      if (
        ...
        setStoredUser((prev) => {
          return {
            ...
            stepsCompleted:
              prev?.stepsCompleted !== undefined ? prev?.stepsCompleted : 0,
          };
        });
  ...
  const handleInput = () => {
    if (textareaRef.current) {
      ...
      textareaRef.current.style.height = `${Math.min(
        ...
        5 *
          parseFloat(getComputedStyle(textareaRef.current).lineHeight || "40"),
      )}px`;
    }
  };
            ...
            {message.length > 0 && (
              <div className="absolute -z-10 -bottom-[6.4rem] bg-[#141415] left-1/2 -translate-x-1/2 w-fit whitespace-nowrap h-full text-[#F2BD57]">
                <p className="text-[#bbbbbb] text-xs">
                  GitDocs AI can make mistakes. Please review the changes before
                  saving.
                </p>
              </div>
            )}
