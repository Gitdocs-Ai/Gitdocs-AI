...
export function checkMobile(): boolean {
    try {
        ...
        if (
            ...
            localStorage.getItem("forceDesktopView") === "true"
        ) {
            return false;
        }
        ...
        if (
            desktopIndicators.some((indicator) => userAgent.includes(indicator))
        ) {
        ...
        const mobilePhoneRegex = new RegExp(
            mobilePhoneIndicators.join("|"),
            "i",
        );
...
export function getDeviceInfo(): DeviceInfo {
    try {
        ...
        // Check for mobile indicators
            ...
            /iphone|ipod|android|blackberry|opera mini|opera mobi|windows phone|mobile/i.test(
                userAgent,
            )
        ) {
            deviceType = "mobile";
