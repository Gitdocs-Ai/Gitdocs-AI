...
const UpdateReadmePage: React.FC<UpdateReadmePageProps> = ({ doc_name }) => {
    ...
    useEffect(() => {
        const allReposotories = JSON.parse(
            localStorage.getItem("repositories") as string,
        );
        ...
        const value = allReposotories.some(
            (repo: any) => repo.name === doc_name,
        );
    ...
    useEffect(() => {
        ...
        if (doc_name) {
            const docNameStr = Array.isArray(doc_name)
                ? doc_name[0]
                : doc_name.toString();
            setNavbarTitle(
                doc_name === "#Chat-with-GitDocs-AI-Assistant#"
                    ? "Chat with GitDocs AI"
                    : docNameStr,
            );
            setIsSidebarUsed(
                doc_name === "#Chat-with-GitDocs-AI-Assistant#" ? true : false,
            );
        }
